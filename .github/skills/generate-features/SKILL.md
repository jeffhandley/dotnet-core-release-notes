...
If a change depends on public APIs, use `dotnet-inspect` to confirm the API exists in the actual build. Missing or reverted APIs should be scored down or excluded.

#### Running `dotnet-inspect`

The tool runs via `dnx` (like `npx` for .NET). No installation needed.

First, get the correct package versions from `build-metadata.json` (generated by `release-notes-gen generate build-metadata`). Then query the NuGet packages directly:

```bash
FEED="<nuget.source from build-metadata.json>"
VER="<nuget.packages version from build-metadata.json>"

# Search for a type by name pattern
...
Use the appropriate ref pack for each product area: `Microsoft.NETCore.App.Ref` for runtime, `Microsoft.AspNetCore.App.Ref` for ASP.NET Core, `Microsoft.WindowsDesktop.App.Ref` for WPF/WinForms, or `Microsoft.EntityFrameworkCore` for EF Core.

If the API cannot be found, score the change down sharply — it likely means the API is internal, was reverted, or is named differently than the PR title suggests. See [api-verification.md](../release-notes/references/api-verification.md) for the full verification workflow.
