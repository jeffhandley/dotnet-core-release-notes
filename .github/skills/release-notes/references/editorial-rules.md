...
**Don't scatter related sections.** The most common ordering failure is splitting items about the same area across the document — for example two `Async …` sections with unrelated material between them. Related sections must be **adjacent**. If a milestone has a second section on an area already covered, move it next to the first (or merge them per the clustering rules under *What to include*).
